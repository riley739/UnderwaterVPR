...
def build_prediction_image(img, images_paths, preds_correct):
...
#TODO: Figure out
def save_preds(predictions, eval_ds, log_dir, img, val, save_only_wrong_preds=None, use_labels=True):
    ...
    for query_index, preds in enumerate(tqdm(predictions, desc=f"Saving preds in {viz_dir}")):
        ...
        prediction_image = build_prediction_image(img, list_of_images_paths, preds_correct)
        ...
        #     output_path=viz_dir / f"{val:03d}.txt",
