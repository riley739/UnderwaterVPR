...
class HoloOceanRerankTestDataset(BaseTestDataset):
    def __init__(
        self,
        **kwargs
    ):
        super().__init__(**kwargs)
        self.db = pd.read_csv(self.dataset_path / "Dataframes" / f"db.csv")
        self.query = pd.read_csv(self.dataset_path / "Dataframes" /  f"q.csv")
    ...
    def get_images(self):
        db = pd.read_csv(self.dataset_path / "Dataframes" / f"db.csv")
        query = pd.read_csv(self.dataset_path / "Dataframes" / f"q.csv")

        db_images = db["name"].tolist()
        db_images = [self.dataset_path /"Images" / "Database" / db for db in db_images]
        query_images = query["name"].tolist() 
        query_images = [self.dataset_path / "Images" / "Query" / query for query in query_images]
