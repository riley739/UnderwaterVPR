...
def evaluate(config):
    ...
    while True:
        ...
        if img is not None:
            ...
            query_descriptor = get_descriptor(model, img, config["device"])
            ...
            save_preds(
                    predictions[:, : config["evaluation"]["num_preds_to_save"] ], dataset, log_dir, img, count, config["evaluation"]["save_only_wrong_preds"], config["evaluation"]["use_labels"]
            )
            count += 1
